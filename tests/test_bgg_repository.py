...
def test_association_round_trip_and_update(
    ...
    pending = BggAssociation(
        game_id, True, BggMatchState.PENDING, "Carcassonne"
    )
...
def test_association_requires_an_existing_game(tmp_path: Path) -> None:
    ...
    database.initialize()
    association = BggAssociation(
        999, True, BggMatchState.PENDING, "Missing"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        BggAssociation(
            1, True, BggMatchState.PENDING, "Game", match_confidence=1.1
        ),
        BggAssociation(
            1, True, BggMatchState.PENDING, "Game", year_published=0
        ),
    ],
)
